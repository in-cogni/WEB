...
function SwitchBackground()
{
    ...
    /*if (switchButton.attributes.src.nodeValue == 'img/moon.png')
    {
        ...
        document.body.style.background = "linear-gradient(to bottom left, #240672, black, black)";
        document.body.style.color = "white";
    }
    else
    {
        switchButton.attributes.src.nodeValue = 'img/moon.png';
        document.body.style.background = "linear-gradient(to bottom left, #FFDD40, wheat, wheat)";
...
function setImage()
{
    ...
    //document.getElementById("photo").imgElement.src = splitted_filename[splitted_filename.length - 1];
...
document.body.onload = function tick_timer()
{
    ...
    document.getElementById("weekday").textContent = time.toLocaleDateString('en', { weekday: 'long' });
...
function tickCountdown()
{
    ...
    let duration = Math.abs(targetTime - now);    //Разность дат вычисляется в формате Timestamp
